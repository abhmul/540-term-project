...
content_img = content_image_loader("content.jpg").type(dtype)

# desired size of the output image
imsize = (content_img.size(3), content_img.size(2))
print("Using content image of size {}".format(imsize))
...
def image_loader(image_name):
    image = Image.open(image_name)
    # image = image.resize(imsize, resample=Image.BILINEAR)
...
def run_style_transfer(cnn, content_img, style_img, input_img, num_steps=1000,
    ...
    print("Best Scores - Content: {} - Style: {}".format(best_scores[0], best_scores[1]))
